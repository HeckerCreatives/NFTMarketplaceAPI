...
exports.protectuser = async (req, res, next) => {
    ...
    if (!token){
        return res.status(401).json({ message: 'Unauthorized', data: "You must be logged in!" });
    }
    ...
    try{
        ...
        if (decodedToken.auth != "user"){
            return res.status(401).json({ message: 'Unauthorized', data: "Access denied. User account required." });
        }
        ...
        if (!user){
            ...
            return res.status(401).json({ message: 'Unauthorized', data: "User not found!" });
        }

        if (user.status != "active"){
            res.clearCookie('sessionToken', { path: '/' })
            return res.status(401).json({ message: 'failed', data: `Your account has been ${user.status}! Please contact support for more details.` });
        }

        if (decodedToken.token != user.webtoken){
            res.clearCookie('sessionToken', { path: '/' })
            return res.status(401).json({ message: 'duallogin', data: `Your account has been opened on another device! You will now be logged out.` });
        }
        ...
        return res.status(401).json({ message: 'Unauthorized', data: "Invalid session!" });
    }
}
